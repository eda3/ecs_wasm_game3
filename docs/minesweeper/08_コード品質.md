...
- **未使用フィールドの処理**
  - 構造体フィールドが未使用の場合も同様に、アンダースコアプレフィックスを付ける
    ```rust
    // 避けるべき例: 未使用フィールドをそのままにする
    pub struct GameTime {
        elapsed: f32,
        delta: f32, 
        time_scale: f32,
        _start_time: f64, // アンダースコアプレフィックスで意図的に未使用であることを示す
        last_update: f64,
        frame_count: u64,
    }
    
    // 良い例: 未使用フィールドにアンダースコアプレフィックスを付ける
    pub struct GameTime {
        elapsed: f32,
        delta: f32,
        time_scale: f32,
  ...
  - 実装も対応して更新する
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            delta: 0.0,
            time_scale: 1.0,
            _start_time: current_time, // フィールド名に合わせる
  ...
  - このパターンの利点:
    - コンパイラ警告が出なくなる
    - フィールドが将来必要になった場合、アンダースコアを削除するだけでよい
    - 他の開発者にそのフィールドが現在は使用されていないことを明示できる
  
  - 注意点:
    - `#[derive(Debug, Clone)]`などのマクロが適用されている場合は、フィールドが使われていなくても警告は抑制されることがある
    - しかし、将来の保守性のためにアンダースコアプレフィックスを付けることが推奨される

### 1.6 不必要な可変性を避ける
- **必要な場合のみ`mut`キーワードを使用する**
  - Rustでは変数はデフォルトで不変（immutable）であり、変更が必要な場合のみ明示的に`mut`を付ける
    ```rust
    // 良い例: 変更が必要ない変数には`mut`を付けない
    let entity = Entity::new();
    
    // 避けるべき例: 変更しない変数に不必要な`mut`を付ける
    let mut entity = Entity::new(); // 警告: variable does not need to be mutable
    ```
  
  - 不必要な可変性を避ける理由:
    - コードの意図が明確になる（どの変数が変更されるか一目でわかる）
    - コンパイラによる最適化の機会が増える
    - 並行処理の際の安全性が向上する
    - 不変性はRustの安全性モデルの基本原則

  - メソッドチェーンパターンでの適切な可変性
    ```rust
    // 良い例: ビルダーパターンでは中間変数ではなく、最終的な変数にのみ`mut`を使用
    let snapshot = LocalEntitySnapshot::new(entity_id, timestamp)
        .with_position([10.0, 20.0, 0.0])
        .with_rotation([0.0, 0.0, 0.0, 1.0]);
    
    // 避けるべき例: 中間変数に不必要な`mut`を使用
    let mut snapshot = LocalEntitySnapshot::new(entity_id, timestamp);
    snapshot = snapshot.with_position([10.0, 20.0, 0.0]);
    snapshot = snapshot.with_rotation([0.0, 0.0, 0.0, 1.0]);
    ```

  - `&mut self`と`&self`の適切な使い分け
    ```rust
    // 良い例: 状態を変更するメソッドのみ`&mut self`を使用
    impl Player {
