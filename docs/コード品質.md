...
### 6.1 物理エンジン関連の修正
- `PhysicsEntity` には `Clone` トレイトを実装し、値のコピーを容易にする
- `SpatialGrid` と `CollisionFilter` にも `Clone` トレイトを実装する
- `SpatialGrid` の `add_entity` メソッドは、正しくは `insert_entity` であることに注意
- `CollisionFilter` の `set_entity_category` および `set_entity_mask` メソッドは、正しくはそれぞれ `set_category` および `set_mask` である
- `PhysicsStep.update()` は `(usize, f64)` のタプルを返すため、それに応じた処理が必要
- 衝突ペア（collision pairs）はタプル `(u32, u32)` であり、これらの値は `pair.0` および `pair.1` としてアクセスする
- `generate_collision_pairs` 関数の呼び出し時、`collision_filter` 引数は `Option<CollisionFilter>` 型として渡す必要がある
