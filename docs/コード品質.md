...
- トレイト境界は明示的に記述する
  ...
  pub fn query<T>(&mut self) -> Query<T> 
  where
      T: 'static, // `component::Component`が必要なのに不足
  {
      // ...
  }
  ```
...
- 共通の型制約をトレイト境界として抽出
  ```rust
  // 良い例: 共通の型制約をwhereで明示
  pub fn perform_action<T, U>(value: T, other: U) -> Result<(), Error>
  where
      T: Component + Clone,
      U: AsRef<str> + Send,
  {
  ...
  // 避けるべき例: インラインでの複雑な型制約
  pub fn perform_action<T: Component + Clone, U: AsRef<str> + Send>(
      value: T, other: U
  ) -> Result<(), Error> {
      // ...
  }
